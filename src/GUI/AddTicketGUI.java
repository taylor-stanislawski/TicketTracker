...
public class AddTicketGUI {
	/**
	 * @wbp.parser.entryPoint
	 */
	public void AddTicketGUI(ArrayList<Ticket> itemList, Connection conn){
		JPanel contentPane;
		...
		*/
		...
		btnNewButton_1.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				//btnAddButton.AddMenuItemFunction re = new AddMenuItemFunction(conn);
				//re.FoodItems.clear();
				frame.dispose();
			}
		});
		...
		btnEdit.addMouseListener(new MouseAdapter() {
			...
			//	}
				...
				Ticket thisItem = itemList.get(thisId);
				...
				//if(re.idExists(thisItem.getId(), conn)==true){//tests to see if the id exists in our database
					...
					//String amounts = thisItem.getAmounts();
					...
					/*for (int i = 0; i < amounts.length(); i++) { 
						a = Character.getNumericValue(amounts.charAt(i));
						while (a==thisItem.getId()) {
							b=Character.getNumericValue(amounts.charAt(i+1));
					...
					JLabel lblNewLabel = new JLabel("Enter the new info for  ticket # " + thisItem.getId());
					...
					btnSaveButton.addMouseListener(new MouseAdapter() {
						...
						public void mouseClicked(MouseEvent e) {
							...
							String ticketId = edittextField.getText();
							String newAmount = edittextField_2.getText();
							int editId=9789;
							int amount = 0;
							int id = 0;
						...
						try {
							...
							id = Integer.parseInt(ticketId);
								...
								re.EditItem(id, amount, thisItem.getId(), conn);//run the changes to the function
								removeFrame.dispose();
								frame.dispose();
								...
								btnOkayButton.addMouseListener(new MouseAdapter() {
									...
									public void mouseClicked(MouseEvent e) {
										invalidFrame.dispose();
										re1.AddTicketGUI(itemList, conn);//run our gui again with our new changes
									}
								});
		...
		btnRemove.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
						...
						Ticket thisItem = itemList.get(id);
						System.out.println(id);
						boolean removed = false;//re.RemoveItem(thisItem.getId(), conn);//remove the id
						if (removed==false) {
							JPanel invalidcontentPane;
							JLabel txtYouEnteredAnd;
